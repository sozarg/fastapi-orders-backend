...
# Inicialización del cliente de Xata
try:
    xata = XataClient(
...
# Enums para métodos de entrega y pago
...
async def get_completed_orders():
    ...
    try:
        completed_orders = xata.data().query(
            "orders",
            {
                "filter": {"status": DeliveryMethod.DELIVERY.value},
                "page": {"size": 100}
            }
        )
        ...
        raise HTTPException(status_code=500, detail=f"Error al obtener los pedidos completados: {str(e)}")
